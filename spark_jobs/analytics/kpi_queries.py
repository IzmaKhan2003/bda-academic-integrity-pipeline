...
Working version based on archival job
"""
...
def calculate_all_kpis(spark, mongo_uri):
    """
    ...
    df_attempts = spark.read \
        ...
        .option("collection", "exam_attempts") \
        .load()
    ...
    df_sessions = spark.read \
        ...
        .option("uri", mongo_uri) \
        .option("database", "academic_integrity") \
        .option("collection", "session_logs") \
        .load()
    ...
    kpi_results['accuracy_rate'] = df_attempts \
        .groupBy('student_id', 'exam_id') \
        .agg((sum(when(col('is_correct'), 1).otherwise(0)) / count(lit(1)) * 100).alias('accuracy_rate')) \
        .cache()
    ...
    print("\n6. Calculating answer_similarity_index (this may take a while)...")
    # Optimize the self-join by filtering and repartitioning
    df_attempts_cached = df_attempts.select('student_id', 'exam_id', 'question_id', 'answer_hash').repartition(50).cache()
    
    df_hash_pairs = df_attempts_cached.alias('a1') \
        .join(
            df_attempts_cached.alias('a2'),
            (col('a1.exam_id') == col('a2.exam_id')) &
            (col('a1.question_id') == col('a2.question_id')) &
            (col('a1.answer_hash') == col('a2.answer_hash')) &
            (col('a1.student_id') < col('a2.student_id'))
        ) \
        .select(
    ...
    try:
        pair_count = kpi_results['answer_similarity_index'].count()
        print(f"  ✓ Found {pair_count:,} suspicious pairs")
    except Exception as e:
        print(f"  ⚠ Collusion detection skipped due to memory constraints")
        # Create empty DataFrame with correct schema
        from pyspark.sql.types import StructType, StructField, StringType, LongType
        schema = StructType([
            StructField("student_1", StringType(), True),
            StructField("student_2", StringType(), True),
            StructField("exam_id", StringType(), True),
            StructField("matching_questions", LongType(), True)
        ])
        kpi_results['answer_similarity_index'] = spark.createDataFrame([], schema)
    
    df_attempts_cached.unpersist()
    ...
    kpi_results['session_irregularity_score'] = df_sessions \
        .withColumn(
            ...
            (col('idle_time_seconds') / 7200) * 0.3
        ) \
    ...
    kpi_results['suspicion_confidence'] = suspicion_df \
        .withColumn('normalized_variance', 1 - (col('response_variance') / max_variance)) \
        .withColumn(
            'suspicion_score',
    ...
    for kpi_name, df in kpi_results.items():
        if kpi_name != 'answer_similarity_index':
            ...
            print(f"  {kpi_name}: {record_count:,} records")
...
if __name__ == "__main__":
    ...
    # Create Spark session - run in local mode with more memory
    ...
    # Calculate KPIs
    results = calculate_all_kpis(spark, mongo_uri=mongo_uri)
