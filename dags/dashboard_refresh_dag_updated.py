"""
Updated Dashboard Data Refresh DAG
Includes collusion pairs detection
"""
...
dag = DAG(
    'dashboard_data_refresh_v2',
    default_args=default_args,
    description='Refresh all dashboard data sources every minute',
    schedule_interval='*/1 * * * *',
    catchup=False,
    tags=['dashboard', 'grafana', 'refresh'],
)
...
def refresh_aggregated_collections():
    ...
    print(f"✓ Refreshed aggregated_kpis: {count:,}")
    
    # Refresh high_risk_students
    ...
    pipeline = [
        ...
        {'$unwind': {'path': '$exam_info', 'preserveNullAndEmptyArrays': True}},
        {'$project': {
            'student_id': 1,
            'student_name': {'$ifNull': ['$student_info.name', 'Unknown']},
            'program': {'$ifNull': ['$student_info.program', 'Unknown']},
            'exam_id': 1,
            'exam_name': {'$ifNull': ['$exam_info.exam_name', 'Unknown']},
            'avg_ai_score': 1,
            'avg_response_time': 1,
            'accuracy_rate': 1,
            'total_attempts': 1
    ...
    print(f"✓ Refreshed high_risk_students: {count:,}")
...
def refresh_time_series():
    ...
    print(f"✓ Refreshed time_series_kpis: {count:,}")
...
refresh_collusion_task = BashOperator(
    task_id='refresh_collusion_pairs',
    bash_command='docker exec spark-master python3 /spark/jobs/create_collusion_pairs.py',
    dag=dag,
)

# Task dependencies
refresh_agg_task >> refresh_ts_task >> refresh_collusion_task
